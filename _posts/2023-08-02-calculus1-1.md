...
Suppose that \\(f(x)\leq g(x)\leq h(x)\\) when \\(x\\) is near \\(a\\). Suppose also that \\(\lim_{x\rightarrow a} f(x)=\lim_{x\rightarrow a} h(x)=P\\). Then \\(\lim_{x\rightarrow a} g(x)=P\\).
![image](/assets/images/calculus1.2.png)
