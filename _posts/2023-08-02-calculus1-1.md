...
x=c에서 함수 f의 극한값이 L이라면, x가 c보다 작은 쪽에서 접근할 때와 c보다 큰 쪽에서 접근할 때 모두 f의 함숫값을 L에 가까워지도록 할 것이다. 즉 x=c 지점을 기준으로 양쪽의 극한값이 같다는 의미이다. 이와 같이 극한값은 한쪽 극한만 선택하여 계산할 수 있다. x=c보다 작은 쪽(왼쪽)에서의 극한값을 좌극한, x=c보다 큰 쪽(오른쪽)에서의 극한값을 우극한이라고 부른다. 그리고 x=c에서 좌극한과 우극한 값이 같을 때, 함수 f가 x=c에서 극한값을 가진다고 말할 수 있다.
