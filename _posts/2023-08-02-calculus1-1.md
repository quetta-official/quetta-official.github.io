...
The function \\(f(x)\\) is not defined at \\(x=1\\). But we can calculate the limit value at that point. It is because \\(x\\) constantly approaches \\(1\\) but never be \\(1\\) like I refered. You can know that the graph of \\(f(x)\\) approaches \\(2\\) near \\(x=1\\) intuitively with the Figure 1.1. This result is same as the value calculated from the following expression.\
